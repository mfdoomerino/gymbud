defmodule Gymbud.Queries.ExerciseQueries do
  import Ecto.Query
  alias Gymbud.Exercises.Exercise

  def query_all_exercises(params \\ %{}, preload \\ []) do
    query = from(w in Exercise, preload: ^preload)
  ...
  defp query_by(query, %{"exercise_id" => id} = params) do
    query = from(q in query, where: q.exercise_id == ^id)

    query_by(query, Map.delete(params, "exercise_id"))
  end
